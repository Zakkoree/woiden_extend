...
于是就想起了 VPN 组网，然后就有了 IP4 地址，也不需要再挂 VPN 可以直连，使用的工具是 `zerotier` ，并不需要公网 IP，基于p2p打洞 🕳 技术组成的虚拟局域网，参照官网文档教程配置 hax 就行了，woiden 有点麻烦没必要折腾 (如果是 linux 大佬请随意) 可以在 hax 上启用ssh隧道连接到 woiden `ssh -f -N -g -L 2222:localhost:22 root@woide_IPv6` 或者 hax 跳转方式进行直连，也适用其他被墙或无公网IP服务器 (理论上可行，我也没测试过几次)

配合 `Squid` 工具使用，你会发现原本需要挂 VPN 连接的 VPS 变成了一个免费的 “VPN”(代理服务器) 了 🌐，带宽足够延迟一般，由于网络波动勉强支持4K 🎞，还可以开启BBR提速 📈  </br>
嗯~  反转 :)  <sub>ps:多操作系统支持使用包括手机端</sub></br>
