...
> **Warning** 
> 1. `Github Action` 上的有些IP都被 `Google` ban了，因为有的服务器被人用过了IP被识别为机器人，可能上个人也调用了 `Google reCaptcha` ，所有语音验证随缘，使用 `2Captcha` 和 `YesCaptcha` 图片验证不受影响稳如老狗，建议两个同时使用即稳定也不费钱，或者[托管自己服务器](https://docs.github.com/cn/actions/hosting-your-own-runners/about-self-hosted-runners)</br>
