...
> **Warning** 
> 1. `Github Action` 上的 `Google reCaptcha` 验证基本触发不了， `Github Action` 服务器99%的IP都被 `Google` ban了，因为有的IP被别人使用过（干了啥就不知道了嘿嘿！），所以识别成了机器人，只能使用 `2Captcha` 或 `YesCaptcha` 的API破解验证码，或者托管自己的服务器 [参考教程](https://docs.github.com/cn/actions/hosting-your-own-runners/about-self-hosted-runners)</br>
