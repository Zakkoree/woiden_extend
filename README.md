...
于是就想起了 VPN 组网，然后就有了 IP4 地址，也不需要再挂 VPN 可以直连，使用的工具是 `zerotier` ，并不需要公网 IP，基于p2p打洞 🕳 技术组成的虚拟局域网，参照官网文档教程配置 hax 就行了，woiden 有点麻烦没必要折腾 (如果是 linux 大佬请随意) 可以在 hax 上启用ssh隧道连接到 woiden `ssh -f -N -g -L 2222:localhost:22 root@woiden_IPv6` 进行直连或者直连 hax 跳转 woiden 方式，也适用其他被墙或无公网IP服务器 (理论上可行，我也没测试过几次)
